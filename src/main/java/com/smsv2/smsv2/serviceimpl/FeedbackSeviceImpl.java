...
public class FeedbackSeviceImpl implements FeedbackService {
	...
	public ResponseEntity<?> updateFeedback(int id, FeedbackDTO feedbackDTO) {
		...
		if(feedback.getEmailId().equals(feedbackDTO.getEmail())) {
			feedback.setMsg(feedbackDTO.getMsg());
			feedback.setRating(feedbackDTO.getRating());
			...
			throw new ResourceBadRequestException("your email id "+feedbackDTO.getEmail()+"owner email id"+feedback.getEmailId());
		}
...
//		System.out.println(feedbackDTO.getEmail());
		if ((feedback.getUser().getEmail().equals( feedbackDTO.getEmail()) )|| (admin.isPresent()&&admin.get().getRole().equals("admin"))) {
