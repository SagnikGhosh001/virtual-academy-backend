...
public class InboxServiceImpl implements InboxService {
	...
	public ResponseEntity<?> addInbox(InboxDTO inboxDTO) {
		Student student = studentDao.findByEmail(inboxDTO.getEmail())
				.orElseThrow(() -> new ResourceNotFoundException("student", "email", inboxDTO.getEmail()));
		...
		inbox.setStudentEmail(student.getEmail());
		inbox.setStudentDept(student.getDeptname());
		inbox.setStudentSem(student.getSemname());
		inbox.setStudentReg(student.getReg());
		inboxDao.save(inbox);
		return new ResponseEntity<>(inbox,HttpStatus.OK);
	}
	...
	public ResponseEntity<?> updateInbox(int id, InboxDTO inboxDTO) {
		...
		if (existInbox.getTeacher().getId() == inboxDTO.getTeacherId()) {
			...
			existInbox.setModifiedAt(LocalDateTime.now());
			existInbox.setRead(false);
	...
	@Override
	public ResponseEntity<?> isRead(int id, InboxDTO inboxDTO) {
		Inbox existInbox = inboxDao.findById(id).orElseThrow(() -> new ResourceNotFoundException("feedback", "id", id));
		if (existInbox.getStudent().getId() == inboxDTO.getCurrentId()) {
			existInbox.setRead(true);
			inboxDao.save(existInbox);
			return new ResponseEntity<>(existInbox,HttpStatus.OK);
		} else {
			throw new ResourceBadRequestException("you are not allowed");
