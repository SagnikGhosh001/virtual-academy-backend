...
public class TopicServiceImpl implements TopicService {
	...
	public ResponseEntity<?> addTopic(TopicDTO topicDTO) {
		...
		if(teacher.getSub()==null) {
			throw new ResourceBadRequestException("You are not assigned any subject");
		}
		if (teacher.getSub().contains(sub)) {
			...
			topic.setCreatedby(teacher.getEmail());
			topicdao.save(topic);
	...
	public ResponseEntity<?> updateTopic(int id, TopicDTO topicDTO) {
		...
		if (teacher.getSub().contains(existTopic.getSub())) {
			...
			existTopic.setModifiedAt(LocalDateTime.now());
			existTopic.setModifiedby(teacher.getEmail());
