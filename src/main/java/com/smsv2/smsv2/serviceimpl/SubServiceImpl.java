...
//	@Override
//	public ResponseEntity<List<Sub>> getAllSubByAssignmentId(int id) {
//		List<Sub>sub= subdao.findByTeacherAssignementId(id);
//		return new ResponseEntity<>(sub,HttpStatus.OK);
//	}
	...
	public ResponseEntity<?> addSub(SubDTO subDTO) {
		...
		if (!teacher.getRole().equals("pic")&& teacher.getDept() == null || teacher.getSem()==null) {
		...
		if (sem.getDept().contains(dept)) {
			if (teacher.getRole().equals("pic")) {
				...
				sub.setSem(sem);
				sub.setSemname(sem.getSemname());
//				sub.setTeacherStaticId(teacherChange.getId());
				sub.setDeptname(dept.getDeptname());
				sub.setCreatedat(LocalDateTime.now());
				subdao.save(sub);
				return new ResponseEntity<>(sub,HttpStatus.CREATED);
	...
	public ResponseEntity<?> updateSub(int id, SubDTO subDTO) {
		...
		if (teacher.getRole().equals("pic")) {
			...
			existSub.getAssignment().forEach(a -> a.setSubname(subDTO.getSubname()));
			existSub.getAttendance().forEach(a -> a.setSubname(subDTO.getSubname()));
			existSub.getMarks().forEach(a -> a.setSubname(subDTO.getSubname()));
			existSub.getNotes().forEach(a -> a.setSubname(subDTO.getSubname()));
			existSub.getBook().forEach(a -> a.setSubname(subDTO.getSubname()));
			existSub.getTopics().forEach(a -> a.setSubname(subDTO.getSubname()));
			existSub.getAssignment().forEach(a -> a.getAssignmentUpload().forEach(s->s.setSubjectname(subDTO.getSubname())));
			existSub.setTeacher(teacherChange);
			existSub.setTeacherStaticId(teacherChange.getId());
			existSub.setTeachername(teacherChange.getName());
			existSub.setModifiedat(LocalDateTime.now());
